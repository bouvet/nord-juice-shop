...
Once you've deployed the services (using `./manage-multijuicer.sh up`), make sure to generate and import the CTFd challenges (unless you've disabled the CTFd service). See [Importing challenges to CTFd](#importing-challenges-to-ctfd) below.

### Importing challenges to CTFd
*The CTF flags from JuiceShop must be imported into the CTFd instance before the users can submit their flags. The CSV file that should be imported can be generated by running the script [`generate-challenges.sh`](./generate-challenges.sh)*
...
```bash
./generate-challenges.sh -h
Usage: ./generate-challenges.sh
